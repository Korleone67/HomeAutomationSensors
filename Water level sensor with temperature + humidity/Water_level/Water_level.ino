...
void setup() {
  ...
  gw.sendSketchInfo("Water alarm CV and washing machine", "1.0");
...
void loop() {
  ...
  float waterLevel = (waterSensorValue / (float)1024) * (float)4.2; 
  Serial.println(waterLevel);
  if(waterLevel > 0.15 && lastWaterLevel != waterLevel) {
  ...
  if(lastWaterLevel != waterLevel && waterLevel <= 0.15) {
